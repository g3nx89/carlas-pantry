...
3. **BASH-COMPATIBLE** — Generated hook scripts must work on macOS and Linux. Use `#!/usr/bin/env bash`. Avoid GNU-specific flags (e.g., prefer `grep -E` over `grep -P`).
...
### Config Key to Script Name Mapping

| Config Key | Script Name | Event | Section |
|------------|-------------|-------|---------|
| `spec_protection` | `protect-specs.sh` | PreToolUse | B.1 |
| `language_enforcement` | `enforce-language.sh` | PreToolUse | B.2 |
| `tdd_reminder` | `tdd-reminder.sh` | PreToolUse | B.3 |
| `safety_guards` | `safe-bash.sh` | PreToolUse | B.4 |
| `commit_format` | `conventional-commits.sh` | PreToolUse | B.5 |
| `code_formatting` | `auto-format.sh` | PostToolUse | B.6 |
| `architecture_boundaries` | `check-architecture.sh` | PostToolUse | B.7 |
| `build_output_analysis` | `build-output-analysis.sh` | PostToolUse | B.8 |
| `session_context` | `session-context.sh` | SessionStart | B.9 |
...
- **Template pattern:**
  ...
  set -euo pipefail
  # jq availability check — degrade gracefully if missing
...
- **Template pattern:**
  ```bash
  #!/usr/bin/env bash
  # safe-bash.sh — Block destructive Bash operations
  # Generated by implement skill project setup (Section 1.5b)
  set -euo pipefail
  # jq availability check — degrade gracefully if missing
  if ! command -v jq &>/dev/null; then
  ...
  for pattern in "${BLOCKED_PATTERNS[@]}"; do
    if echo "$COMMAND" | grep -qE "$pattern"; then
      echo "BLOCKED: Destructive operation detected: $COMMAND"
      exit 2
    fi
  done
  ```
...
- **Template pattern:**
  ```bash
  #!/usr/bin/env bash
  # auto-format.sh — Auto-format files after Claude writes them
  # Generated by implement skill project setup (Section 1.5b)
  # NOTE: This is a PostToolUse hook — it receives tool result JSON, not tool input
  set -euo pipefail
  # jq availability check — degrade gracefully if missing
  if ! command -v jq &>/dev/null; then
    exit 0
  ...
  case "$FILE_PATH" in
    *.md|*.json|*.yaml|*.yml|*.lock|*.svg|*.png) exit 0 ;;
  esac
  # FORMATTER_CMD and FILE_GLOB are set by the generator based on detected formatter
  # Example values: FORMATTER_CMD="ktfmt --kotlinlang-style" FILE_GLOB="*.kt"
  FORMATTER_CMD="${FORMATTER_CMD:-}"
  FILE_GLOB="${FILE_GLOB:-}"
  if [ -z "$FORMATTER_CMD" ]; then exit 0; fi
  # Check if file matches the formatter's glob
  if [ -n "$FILE_GLOB" ]; then
    case "$FILE_PATH" in
      $FILE_GLOB) ;;  # matches — continue
      *) exit 0 ;;    # no match — skip
    esac
  fi
  # Format the file (warning only — do not block on formatter errors)
  if ! $FORMATTER_CMD "$FILE_PATH" 2>/dev/null; then
    echo "WARNING: Formatter failed on $FILE_PATH — file left as-is"
  fi
  ```
...
4. **Register in settings.json:**
   ...
   - **Post-write validation:** Read the written `.claude/settings.json` back and verify it parses as valid JSON. If parsing fails, restore from `.claude/settings.json.bak` and log warning "settings.json write produced invalid JSON — restored from backup"
5. **Syntax check:** After generating each hook script, run `bash -n .claude/hooks/{hook_name}.sh` to verify bash syntax. If syntax check fails, delete the generated script, skip its registration in settings.json, and log warning "Hook {hook_name}.sh failed syntax check — skipped"
