...
**Known risk — Stage 2 context load:** Stage 2's coordinator reads up to 8 artifact files (tasks.md, plan.md, spec.md, design.md, data-model.md, contract.md, research.md, test-plan.md) plus launches developer agents per phase. For features with large spec files or many phases, this may approach context limits. Mitigation: crash recovery can reconstruct state from artifacts, and the coordinator processes phases sequentially (not all at once). If this becomes a practical issue, consider splitting Stage 2 into per-phase coordinator dispatches.
