...
| File | Lines | Purpose |
|------|-------|---------|
| `orchestrator-loop.md` | 764 | Dispatch loop (per-phase + linear modes), crash recovery, lock release, v1→v2→v3 state migration, late notification handling, autonomy policy infrastructure checks, context pack protocol, phase loop with VALIDATE_AND_HANDLE, graduated stall response (4-level chained: warn/blockers/scope-reduce/halt), rate limit exemption, per-stage output-decline detection, test-result stall detection (independent counter), plan mutability (task annotation + phase skip), iteration status file, IDENTIFY_STUCK_TASK, EXTRACT_FAILING_TEST_NAMES, error-pattern tracking, question categorization, cross-iteration learning (APPEND_LEARNING) |
