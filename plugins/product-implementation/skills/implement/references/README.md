...
| `prompt-registry.yaml` | ~195 | Machine-readable registry of all 14 prompt templates — template name, agent type, required/optional variables, stage/step usage. Used by coordinators for dispatch compliance verification and by VERIFY_STAGE_PROTOCOL for agent diversity scoring |
| `summary-schemas.md` | 138 | YAML schemas for all 6 stage summaries — base fields table + per-phase naming convention + per-stage field tables with type, required/optional, default, producer, consumer, protocol_evidence schema (Stages 2-5) |
