...
| `stage-1b-probes.md` | ~710 | Stage 1b coordinator: project setup analysis (1.5b), domain detection (1.6), MCP probing (1.6a-1.6d), mobile device check (1.6e), UAT engine strategy + Figma export (1.6ea), plugin check (1.6f), Figma check (1.6g), CLI availability (1.7a), circuit breaker init (1.7b), autonomy policy (1.9a), quality config (1.9b), pre-summary checklist (1.9c), full summary template (1.10) |
...
| `stage-2-execution.md` | 624 | Skill resolution, research context resolution (2.0a), phase loop, CLI test augmenter (Step 2.1a) — dual-model edge-case gap analysis via dedicated script, native test-writer (Step 1.9, MANDATORY enforcement), output verifier (Step 2.5, MANDATORY enforcement), code simplification (Step 3.5), protocol compliance checklist (Section 2.2a, cross-ref to shared file), auto-commit per phase (linear mode only — orchestrator-owned in per-phase mode), batch strategy, execution rules, build verification, build error smart resolution, test count extraction, cli_dispatch_metrics, circuit state propagation, context contributions, autonomy policy checks |
| `stage-2-uat-mobile.md` | ~160 | UAT mobile testing procedure (3-gate check, engine selection — Claude subagent Path A / CLI script Path B, APK build/install, evidence directory, dispatch, result processing via autonomy-policy-procedure, write boundaries) |
