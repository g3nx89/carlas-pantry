...
- **SAFE_ASK_USER shared function**: `orchestrator-loop.md` defines `SAFE_ASK_USER()` with empty response validation, option matching, and text fallback. Called by `orchestrator-loop.md` (needs-user-input relay, crash recovery) and `stage-1-setup.md` (Sections 1.1, 1.5b, 1.9a, 1.9b). Bypassed entirely in ralph mode (auto-resolve guard intercepts first).
