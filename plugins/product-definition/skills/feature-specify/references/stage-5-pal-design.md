...
4. **design-feedback.md is MANDATORY**: NEVER skip — even if PAL fails
...
Load evaluation criteria from: `@$CLAUDE_PLUGIN_ROOT/templates/prompts/pal-spec-eval.md`
...
Output: specs/{FEATURE_DIR}/design-feedback.md

Use Sequential Thinking (if available, 6 thoughts):
1. Correlate requirements to screens
2. Identify uncovered requirements (no screen mapped)
3. Identify uncovered screens (no requirement mapped)
4. {IF Figma: Compare Figma designs to spec requirements}
5. Analyze edge case coverage in UI
6. Self-critique traceability
```
...
test -f "specs/{FEATURE_DIR}/design-feedback.md" || echo "MISSING: design-feedback.md"
```
...
2. `specs/{FEATURE_DIR}/design-feedback.md` exists and has content
...
- PAL Consensus minimum 2 models — fail if < 2
- No model substitution
- design-brief.md is MANDATORY — NEVER skip
- design-feedback.md is MANDATORY — NEVER skip
