...
| `stage-4b-resolution.md` | ~260 | Parse answers, MPA-Triangulation (via cli-dispatch-patterns), spec update, checkpoint |
| `stage-5-validation-design.md` | ~280 | Pre-conditions, CLI multi-stance evaluation (via cli-dispatch-patterns, retry loop), design-brief-generator, gap-analyzer |
...
- `orchestrator-loop.md` references all stage files (1, 2, 3, 4A, 4B, 5, 6, 7, 8) in dispatch order
- `orchestrator-loop.md` references `recovery-migration.md` for crash recovery and state migration
- `orchestrator-loop.md` performs quality checks after Stages 2, 4B, and 5

### Internal Protocol References

- `stage-1-setup.md` loads `figma-capture-protocol.md` for Figma capture (Step 1.9)
- `stage-4a-analysis.md` loads `auto-resolve-protocol.md` (Step 4.1b) and `clarification-protocol.md` (Step 4.2)
- `stage-4b-resolution.md` loads `clarification-protocol.md` (Step 4.3)
...
Pre-condition validation uses `Step N.0`. Stage 4A uses suffix letters for conditional sub-steps: `Step 4.0a` (RTM disposition), `Step 4.0b` (Figma mock gaps).

## Structural Patterns

All stage files follow this structure:
