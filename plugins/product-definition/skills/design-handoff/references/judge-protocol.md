...
| 5 | **GROUP residue** | Via `figma_get_file_for_plugin(depth=figma.query_depth)`: count GROUP-type nodes that are direct children of prepared screen frames. | GROUP count <= `judge.checkpoints.stage_2j.group_max_residue` in config |
...
| 2 | **Component usage** | Via `figma_get_file_for_plugin(depth=figma.query_depth)`: verify new screen uses library components (INSTANCES) not raw frames for UI elements that have component equivalents. | Library components used where available |
