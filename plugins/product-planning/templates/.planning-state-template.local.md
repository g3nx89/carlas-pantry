---
# Planning State File
# This file tracks the state of feature planning and is auto-generated.
# DO NOT manually edit the YAML frontmatter unless recovering from errors.

version: 3
current_phase: "SETUP"
phase_status: "pending"
analysis_mode: null  # complete | advanced | standard | rapid

# MCP tool availability (detected at setup)
mcp_availability:
  sequential_thinking: null
  agent_teams_available: null

# Research MCP availability (detected at setup)
research_mcp_available: null

# CLI integration state (detected in Step 1.6)
cli:
  available: null
  capabilities:
    gemini: null
    codex: null
  roles_deployed: false
  mode: null  # dual | single_gemini | single_codex | disabled
  dispatch_infrastructure:
    dispatch_method: null  # "ntm" | "legacy"
    ntm_available: null
    ntm_script_available: null
    legacy_script_available: null
  consecutive_failures: 0

# Agent Teams state (detected in Step 1.6b, refined in Step 1.10)
agent_teams:
  enabled: false
  display_mode: "in-process"
  active_teams: []

# Team preset state (resolved in Step 1.10)
team_preset: null  # balanced | rapid_team | null (skip)
team_debate_roles: []  # resolved list of roles that use team debates
review_board_enabled: false  # true only for "balanced" preset + YELLOW/RED verdicts

# Pending background agents (for context resume resilience)
# Written before dispatch, removed after completion.
# On context resume, check these before re-dispatching to avoid duplicates.
pending_agents: []
# Format per entry:
#   - phase: "6b"
#     dispatched_at: "2026-03-05T18:59:00Z"
#     expected_artifacts:
#       - ".phase-summaries/phase-6b-summary.md"

# Phase summary file paths (populated as phases complete)
# Migration: v1 state files are auto-migrated on resume.
# See references/orchestrator-loop.md for migration logic.
phase_summaries:
  phase_1: null  # .phase-summaries/phase-1-summary.md
  phase_2: null
  phase_3: null
  phase_4: null
  phase_5: null
  phase_6: null
  phase_6b: null
  phase_7: null
  phase_8: null
  phase_8b: null
  phase_9: null
  phase_10: null

# Completed phases in order
completed_phases: []

# User decisions are IMMUTABLE once saved
# These are never re-asked on resume
user_decisions: {}

# Architecture-specific state
architecture:
  perspectives_evaluated: 0
  selected_perspective: null
  composition_strategy: null
  design_files: []

# CLI Deep Analysis state (Complete/Advanced modes)
deep_analysis:
  perspectives_analyzed: []
  total_dispatches: 0
  failures: []
  convergent_findings: 0
  divergent_findings: 0

# Validation state
validation:
  method: null  # cli_consensus | internal
  score: null
  status: null  # GREEN | YELLOW | RED

# Asset consolidation (Phase 8b)
assets:
  manifest_generated: false
  asset_count: 0
  categories: []
  validated: false
  skipped: false

# Timestamps
timestamps:
  started: null
  setup_completed: null
  research_completed: null
  clarification_completed: null
  architecture_completed: null
  thinkdeep_completed: null
  validation_completed: null
  completion_completed: null
  expert_review_completed: null
  test_strategy_completed: null
  test_coverage_completed: null
  asset_consolidation_completed: null

# Orchestrator metadata
orchestrator:
  version: 3
  delegation_model: "lean_orchestrator"
  coordinator_failures: 0
  summaries_reconstructed: 0
  duplicate_dispatches_detected: 0

---

# Planning State Log

This section contains a human-readable log of planning activity.

## Session Started

- Feature: {FEATURE_NAME}
- Branch: {GIT_BRANCH}
- Timestamp: {ISO_DATE}

---

<!-- Subsequent phases append their logs below -->
