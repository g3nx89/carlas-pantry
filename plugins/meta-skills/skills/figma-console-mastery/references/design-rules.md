...
| 11 | **SHOULD** name components to match codebase naming convention | Code handoff: enables `get_design_context` to produce component names the coding agent can directly map to existing code. Typically PascalCase for React/SwiftUI, PascalCase function names for Compose. Each segment of a slash path should follow the convention (e.g., `ProductCard/Default`). See also SHOULD #5 for slash hierarchy. |
...
| 14 | **SHOULD** prefer UI kit components (M3, Apple, SDS) where applicable | Code handoff: UI kit components get automatic Code Connect on Professional+ plans, providing bidirectional design-code links. Without Enterprise, use Smart Componentization + Handoff Manifest as an alternative (see `workflow-code-handoff.md` TIER System). See also SHOULD #4 for general component reuse. |
