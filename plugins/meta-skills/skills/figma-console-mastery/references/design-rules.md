...
> For Plugin API code patterns, see `plugin-api-foundation.md` (see also `plugin-api-visuals.md`, `plugin-api-advanced.md`).
...
| 11 | **SHOULD** name components to match codebase naming convention | Code handoff: enables design extraction tools to produce component names the coding agent can directly map to existing code. Typically PascalCase for React/SwiftUI, PascalCase function names for Compose. Each segment of a slash path should follow the convention (e.g., `ProductCard/Default`). See also SHOULD #5 for slash hierarchy. |
| 12 | **SHOULD** use lowercase variant property keys matching code props | Code handoff: Figma variant `size=sm` maps cleanly to code prop `size="sm"`. Avoid `Size`, `Type`, `State` — coding agents expect lowercase. |
| 13 | **SHOULD** use `figma_set_description` for naming exceptions only | Code handoff: when a component's Figma name must differ from its code name (e.g., `CTA Button` vs `CallToActionButton`), document the code name in the description. For multi-platform divergence, include platform-specific names. Do not use descriptions for path or prop mapping — the naming convention handles that. |
| 14 | **SHOULD** apply Smart Componentization Criteria before creating custom components | Code handoff: componentize only elements meeting ALL 3 gates — recurrence (3+ screens), behavioral variants, codebase match. This targets effort on high-value components (typically 5-8) rather than exhaustive componentization. See `workflow-code-handoff.md` Smart Componentization Criteria. |
| 15 | **SHOULD** limit component descriptions to non-visual information only | Descriptions should contain ONLY: (1) purpose/intent — one line, when/why to use it; (2) interaction behavior — tap targets, navigation, toggle actions; (3) animations — pulsing, crossfade, transition sequences; (4) non-obvious constraints — API limitations, conditional property visibility. Remove all visual specs (hex colors, px sizes, font families/weights, padding, cornerRadius) — the canvas is the source of truth for those. Visual specs in descriptions create maintenance burden and stale data. Complementary to SHOULD #13 (naming exceptions in descriptions). |
