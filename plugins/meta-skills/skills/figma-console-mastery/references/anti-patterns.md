...
| `"object is not extensible"` on VectorNode | `strokeCapEnd`/`strokeCapStart` don't exist on VectorNode — use `setVectorNetworkAsync` with per-vertex `strokeCap` |
...
| `node.vectorNetwork = {...}` throws in dynamic-page mode | Synchronous assignment to `vectorNetwork` fails with `"Cannot call with documentAccess: dynamic-page. Use node.setVectorNetworkAsync instead."` when `figma.loadAllPagesAsync()` has been called | Always use `await node.setVectorNetworkAsync({ vertices, segments, regions })` in dynamic-page mode |
| `"object is not extensible"` on VectorNode strokeCap properties | Setting `v.strokeCapEnd = 'ARROW_EQUILATERAL'` or `v.strokeCapStart = 'NONE'` on a VectorNode — these properties do NOT exist on VectorNode. The Plugin API sandbox seals node objects, so adding non-existent properties throws a TypeError | Use `await node.setVectorNetworkAsync(...)` with per-vertex `strokeCap`: `vertices.map((v, i) => ({ ...v, strokeCap: i === lastIdx ? 'ARROW_EQUILATERAL' : 'NONE' }))`. Direct `strokeCapEnd`/`strokeCapStart` properties exist only on `LineNode` |
